 ...
 * 转换流时常用的字符流实现类，它们是一对高级流，实际开发中我们不会直接
 * 使用它们，但是它们在流连接中是非常重要的一环。
 *
 * 因为其他高级字符流都有一个共同特点，在流连接中只能连接其他的字符流
 * 而真实读写设备的低级流通常都是字节流，这会导致这些高级字符流不能直接
 * 连接字节流，而转换流就在其中起到“转换器”作用，它可以连接字节流
 * 并且它又是字符流。其他的字符流可以连接它，而它就负责将字符转换为字节
 * */
public class OSWDemo {
    public static void main(String[] args) throws IOException {
        //向demo.txt文件中写入字符串
        FileOutputStream str=new FileOutputStream("demo.txt");
        OutputStreamWriter st=new OutputStreamWriter(str,"utf-8");
        st.write("夜空中最亮的星，是否听清，");
        st.write("那仰望的人，心底的孤独和叹息，夜空中最亮的星");
        st.write("哼哼哈嘿，快使用双截棍！！！");
